...
DFCompositePSOutput main(DFCompositePSInput input)
{
	...
	const float3 directLighting =
		DFCompositeTexture5.SampleLevel(DFCompositeSampler5, uv, 0.0f).xyz +
		DFCompositeTexture11.SampleLevel(DFCompositeSampler11, uv, 0.0f).xyz;
	const float3 bloomColor = DFCompositeTexture4.Sample(DFCompositeSampler4, uv).xyz;

	const float3 litBase = baseColor.xyz * directLighting;
