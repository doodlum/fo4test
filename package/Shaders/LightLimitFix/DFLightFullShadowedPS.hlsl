// Compile-only candidate for the FO4 PreNG full-shadowed DFLight family.
// This is not runtime-bound. It preserves the observed vanilla resource shape
// while adding LLF b3/t35-t37 consumption for metadata validation.
...
float3 AccumulateLLFDFLight(float2 uv, float viewZ, float3 positionWS, float3 normalWS)
{
	...
	const uint lightCount = min(LightLimitFix::GetStrictLightCount() + clusteredLightCount, 32u);
	float3 result = 0.0f;
