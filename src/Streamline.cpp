...
void Streamline::Upscale(Texture2D* a_dilatedMotionVectorTexture, float2 a_jitter, float2 a_renderSize, uint a_qualityMode)
{
...

	{
		...
		sl::Resource colorIn = { sl::ResourceType::eTex2d,reinterpret_cast<ID3D11Texture2D*>(mainTexture.texture), 0 };
		sl::Resource colorOut = { sl::ResourceType::eTex2d, reinterpret_cast<ID3D11Texture2D*>(mainTexture.texture), 0 };
